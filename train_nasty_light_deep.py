...
if __name__ == "__main__":
    ...
    #Resize layer 
    layer_size = num_class
    if args.layer_size is not None:
        layer_size = args.layer_size
    
    # Adds resized layer / removes teacher initalization
    in_features = model.linear.in_features    
    model.linear = nn.Linear(in_features , layer_size)
    
    # Add additional FC layer to end of model
    model = nn.Sequential(model, nn.Linear(layer_size, num_class))
    model = model.cuda()
    print(model)
    # ############################### Optimizer ###############################
